...
teams_parks_table <- read.csv("xxxxx.csv")
...
test <- read.csv("xxxxx.csv")

#write the existing data set back to the working directory as a backup
write.csv(test, file="xxxx.prev.csv", row.names=FALSE, col.names=TRUE, na="")

#reformat date column
#test$Date<-strftime(test$Date,"%m/%d/%Y")

#rename columns to match query and existing Tableau data

colnames(test)<-c("game_id", "Home", "Away", "Wind Direction", "Wind MPH", "Description", "Outcome", "Name", "MLBAM_ID_B", "stand", "yearid", "hit_x", "hit_y", "distance", "angle", "RunValue", "hit_x_1", "hit_y_rotated", "Pitcher Name", "MLBAM_ID_P", "Date", "Velocity", "Horizontal Location", "Vertical Location", "Balls", "Strikes", "Pitch Type", "Throws", "Batted_Ball_Type", "Zone", "League", "Division", "Outcome_Type")
...
colnames(res2)<-c("game_id", "Home", "Away", "Wind Direction", "Wind MPH", "Description", "Outcome", "Name", "MLBAM_ID_B", "stand", "yearid", "hit_x", "hit_y", "Distance (ft.)", "Angle", "RunValue", "hit_x_1", "hit_y_rotated", "Pitcher Name", "MLBAM_ID_P", "Date", "Velocity", "Horizontal Location (ft.)", "Vertical Location (ft.)", "Balls", "Strikes", "Pitch Type", "Throws", "Batted_Ball_Type", "Zone", "League", "Division", "Outcome Type")

#export and overwrite the updated data set to folder for uploading into Tableau - export as an Excel document

write.csv(res2, file="xxxxx.csv", row.names=FALSE, col.names=TRUE, na="")